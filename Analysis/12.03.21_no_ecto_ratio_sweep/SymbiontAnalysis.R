...
ggplot(data=initial_data, aes(x=update, y=symHostedCount, group=HRSR, colour=HRSR)) + labs(title=("Ratio of resources received by host (HR) versus free-living symbionts (SR)")) + ylab("Count of Endosymbionts") + xlab("Updates") + stat_summary(aes(color=HRSR, fill=HRSR),fun.data="mean_cl_boot", geom=c("smooth"), se=TRUE) + theme(panel.background = element_rect(fill='white', colour='black')) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + guides(fill="none") +scale_colour_manual(name="Host Resources and Symbiont Resources",values=viridis(25)) + scale_fill_manual(values=viridis(25))
